# Shrike Lite: 8-Port FPGA I/O Expander
...
| :---: | :--- | :--- |
| `0x1` | **Lower DIR** | [cite_start]Set Directions for Pins [3:0] (1=Input, 0=Output) [cite: 103] |
| `0x2` | **Upper DIR** | [cite_start]Set Directions for Pins [7:4] (1=Input, 0=Output) [cite: 103] |
| `0x3` | **Lower DATA**| [cite_start]Drive logic levels (High/Low) for Pins [3:0] [cite: 103] |
| `0x4` | **Upper DATA**| [cite_start]Drive logic levels (High/Low) for Pins [7:4] [cite: 103] |

### Output Packet (FPGA -> RP2040)
During every SPI transfer, the FPGA shifts out an 8-bit byte reflecting the **current physical logic state** of the 8 GPIO pins. [cite_start]This allows the RP2040 to "poll" the inputs while sending new output commands[cite: 103].
...
| `rst_n` | In | [cite_start]System Reset (Active Low) [cite: 129] |
...
### Target GPIO Mapping
[cite_start]To maintain independent control, each bit is mapped to its own physical GPIO resource in the I/O Planner[cite: 122]:
...
> [cite_start]**Note:** On reset, the FPGA defaults all pins to **Input mode** to prevent accidental short circuits[cite: 103].
